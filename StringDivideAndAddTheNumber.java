...
public class Hello {
    public static void main(String[] args) {
        ...
        String str1 = sc.next(); // Main string
        String[] str2 = sc.next().split("\\)"); // Split input based on ')'
        ...
        for (String s : str2) {
            ...
            int val = Math.abs(Integer.parseInt(s)); // Convert to int and take absolute value

            // Calculate the end index for substring
            int endInd = ind + val;
            if (endInd > str1.length()) endInd = str1.length(); // Avoid IndexOutOfBounds

            // Extract substring and convert to long
            String sub = str1.substring(ind, endInd);
            long num = Long.parseLong(sub);

            sum += num;
            ind = endInd; // Move index forward
